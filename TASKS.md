...
- [x] **Navbar `brandNote` centering fix** — `packages/ui/src/layout/Navbar.tsx`'s brand block rendered the optional tagline-above-logo (`brandNote`) left-aligned, looking lopsided above the centered business name. Fixed with `alignItems: 'center'`/`textAlign: 'center'` on the shared `Navbar` component — fixes every business using `brandNote` (Jagdamb Creation and Swami Hair Salon both), not a per-business patch. ✅ Done
