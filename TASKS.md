...
## WhatsApp checkout fallback + demo business (Rupali Imitation Jewellery)
...
- [x] **Demo business: `rupali-imitation-jewellery`** — a Tier 1, bilingual (en/mr — [0008](docs/adr/0008-per-business-bilingual-content.md)), pure-retail (`commerceEnabled: true`, `bookingEnabled: false`) imitation jewellery shop, Kolhapur-themed. 6 products (Kolhapuri saaj, kundan bridal set, jhumkas, maang tikka, bangles, temple choker), bilingual gallery/testimonials/FAQ/team content, custom maroon/gold theme, own SVG logo/favicon/product/gallery art. Registered in `JsonDataSource`/`businesses/index.json`. `Product.name`/`description` stay English-only — a pre-existing, documented v1 scope cut (`packages/types/src/product.ts`), not something this task worked around. ✅ Done (verified via a real `JsonDataSource` smoke test exercising every collection for this business, plus a full `apps/website` production build with the new data loaded; **not** verified in an actual browser — no headless-browser driver available in this environment, see chat history)
